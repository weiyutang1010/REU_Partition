...
def probability(D, x):
    ...
    # return p
    # return ((1/6) ** 4) * (.25 ** 4) # change
    return ((1/6) ** 3) * (.25 ** 3) # change
...
def sampling_test(n, k, y):
    # Given a distribution of length n
    # D = np.array([[.25,.25,.25,.25] for _ in range(n)]) # random distribution
    # D = np.array([[1/6,1/6,1/6] for _ in range(n)]) # random distribution
    # print(D)
    ...
    for _ in range(k):
        seq = ['A'] * n
        for j in unpaired:
            seq[j] = np.random.choice(['A','C','G','U'], p=[.25, .25, .25, .25])
        for i, j in paired:
            chosen = np.random.choice(['CG','GC','AU','UA', 'GU', 'UG'], p=[1/6,1/6,1/6,1/6,1/6,1/6])
            seq[i] = chosen[0]
            seq[j] = chosen[1]
...
if __name__ == '__main__':
    ...
    y = "(((...)))"
    n = len(y)
    with open(f'samples_y/n{n}.txt', 'w') as f:
        sys.stdout = f
        print(f"n: {n}, k: {args.k}, y: {y}")
        sampling_test(n, args.k, y)
